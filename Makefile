...
ci:
	npm install -g elm && elm-package install -y && cd tests && elm-package install -y && export PATH=$PATH:/home/travis/build/cmloegcmluin/scalar-set-theory/tests/elm-stuff/packages/rtfeldman/node-test-runner/3.0.0/bin && elm-test && elm-reactor -p 8001 & cd ../integration-tests && npm i && npm test
